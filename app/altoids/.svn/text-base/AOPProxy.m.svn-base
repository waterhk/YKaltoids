...
{    
    ...
    if ( [parentObject respondsToSelector:aSelector] ) {
        ...
        if (![NSStringFromClass([parentObject class]) isEqualToString:@"YAServerTransport"]) {
            //        @autoreleasepool {
        ...
        if (timeInterval > 1) {
            NSLog(@"[PERF_WARNING]%@ took %1.1f seconds",NSStringFromSelector(aSelector),timeInterval);
        }
